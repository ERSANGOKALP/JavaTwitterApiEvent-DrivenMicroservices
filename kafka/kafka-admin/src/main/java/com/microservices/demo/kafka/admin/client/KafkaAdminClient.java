...
public class KafkaAdminClient {
    ...
    public void checkTopicsCreated(){
        ...
        Long slipTimeMs = retryConfigData.getSleepTimeMs();
        for (String topic: kafkaConfigData.getTopicNamesToCreate()) {
            while(!isTopicCreated(topics,topic)) {
                checkMaxRetry(retryCount++,maxRetry);
                sleep(slipTimeMs);
                slipTimeMs *= multiplier;
                topics = getTopics();
            }
        }
    }
    ...
    private void sleep(Long slipTimeMs) {
        try {
            Thread.sleep(slipTimeMs);
    ...
    private void checkMaxRetry(int retry, Integer maxRetry) {
        if (retry > maxRetry){
            throw new KafkaClientException("Reach max number of retry for reading kafka topic(s)!");
        }
    }
